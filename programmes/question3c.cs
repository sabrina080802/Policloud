...
class occurences
{
    static void Main()
    {
        ...
        // On crée un dictionnaire de correspondances :
        Dictionary<String, List<String>> correspondances = new Dictionary<string, List<string>>();

        // On applique les étapes
        List<String> listeMotsEtapes = Traiter(listeMotsVides, correspondances);

        // On compte les occurences 
        Dictionary<String, int> occurences = CompterOccurences(listeMotsEtapes);

        // On reconstitue le dictionnaire avec les correspondances
        Dictionary<String, int> occurencesCorrespondances = ReconstituerCorrespondances(occurences, correspondances);

        // Affichage des résultats
        // AfficherDictionnaire(occurencesCorrespondances);

        // Creation du chemin du fichier CSV
        String csvPath = "./../csv/"+ mot.Split('.')[0]+".csv";

        // Enregistrement en CSV
        EnregistrerCSV(occurencesCorrespondances, csvPath);
    ...
    static List<String> Traiter(List<String> mots, Dictionary<String, List<String>> corrrespondances) {
        ...
        foreach(String mot in mots) {
            ...
            if (mot.Length > 1) {
                ...
                AjouterCorrespondance(mot, motTraite, corrrespondances);
    ...
    static void AjouterCorrespondance(String mot, String racine, Dictionary<String, List<String>> correspondances) {
        
        // Récupération ou création de la liste
        List<String> liste;
        if (correspondances.ContainsKey(racine)) {
            // Racine déjà dans le dictionnaire
            liste = correspondances[racine];
        } else {
            // Racine pas dans le dictionnaire
            liste = new List<String>();
        }

        // Ajout du mot à la liste si il n'y est pas déjà
        if (!liste.Contains(mot)) {
            liste.Add(mot);

            // Ajout du mot au dictionnaire
            if (correspondances.ContainsKey(racine)) {
                correspondances[racine] = liste;
            } else {
                correspondances.Add(racine, liste);
            }
        }

    }

    static string MotLePlusCourt(List<String> mots) {
        if (mots.Count > 0) {
            string resultat = mots[0];
            for (int i = 1; i<mots.Count; i++) {
                if (mots[i].Length < resultat.Length) {
                    resultat = mots[i];
                }
            }
            return resultat;
    ...
    static Dictionary<String, int> ReconstituerCorrespondances(Dictionary<String, int> racines, Dictionary<String, List<String>> correspondances) {

        Dictionary<String, int> resultat = new Dictionary<string, int>();

        foreach(KeyValuePair<String, int> kvp in racines) {
            resultat.Add(MotLePlusCourt(correspondances[kvp.Key]), kvp.Value);
        }
    ...
    static void EnregistrerCSV(Dictionary<String, int> contenu, String path) {

        String resultat = "";

        foreach(KeyValuePair<String, int> kvp in contenu) {
            // Pour chaque lignes
            resultat+=kvp.Key+','+kvp.Value+'\n';
        }
